 ...
 *   label = @Translation("DDBgo Aggregator Bestand nodes"),
 *   description = @Translation("DDBgo Bestände which are linked to Aggregator, used in Aggregator's search"),
 *   stages = {
 *     "add_properties" = 20,
 *   },
 *   locked = FALSE,
 ...
 * )
 */
class AggregatorBestandProcessor extends ProcessorPluginBase
{
  ...
  public function getPropertyDefinitions(DatasourceInterface $datasource = NULL)
  {
    ...
    if ($datasource && $datasource->getEntityTypeId() === 'node') {
      $definition = [
        'label' => $this->t('DDBgo Aggregator Bestand nodes'),
        'description' => $this->t("DDBgo Bestände which are linked to Aggregator, used in Aggregator's search"),
        'type' => 'entity:node',
        'processor_id' => $this->getPluginId(),
        'is_list' => TRUE,
      ];
      $properties['ddbgo_aggregator_bestand'] = new EntityProcessorProperty($definition);
      $properties['ddbgo_aggregator_bestand']->setEntityTypeId('node');
    }
  ...
  public function addFieldValues(ItemInterface $item)
  {
    ...
    foreach ($item->getFields() as $field) {
      ...
      if ($datasource
        ...
        && $direct === 'ddbgo_aggregator_bestand') {
