...
check(tip.hidden && tip.textContent.includes('Hilfetext'), 'Description present but hidden before JS');
const stylesheets=Array.from(document.querySelectorAll('link[rel="stylesheet"]'));
stylesheets.forEach(link=>{link.disabled=true});
check(tip.getBoundingClientRect().height===0, 'HTML hides tooltip even before CSS loads');
stylesheets.forEach(link=>{link.disabled=false});
await delay(200);
const beforeAttach=document.getElementById('outside').getBoundingClientRect().top;
check(tip.getBoundingClientRect().height===0, 'Delayed behavior does not flash help text');
behavior.attach(document);behavior.attach(document);
check(tip.hidden, 'Tooltip initially closed');
check(document.getElementById('outside').getBoundingClientRect().top===beforeAttach, 'Attaching behavior does not shift the form layout');
...
const ajax=document.createElement('div');ajax.innerHTML=markup('ajax-tip');fixtures.append(ajax);
check(document.getElementById('ajax-tip').getBoundingClientRect().height===0, 'AJAX help is hidden before attach');
behavior.attach(ajax);
